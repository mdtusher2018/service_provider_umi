...
class _CallScreenState extends ConsumerState<CallScreen>
  ...
  /// The actual Agora channel name. Falls back to a generated one.
  String get _resolvedChannelId => (widget.channelId?.isNotEmpty == true)
      ? widget.channelId!
      : 'call_${widget.contactId}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  void initState() {
    ...
    // Incoming call → wait for user to tap Accept in the UI.
    if (!widget.isIncoming) {
      _initAgoraAndCall();
    }
  }
  ...
  Future<void> _initAgoraAndCall() async {
    ...
    try {
      ...
      _engine!.registerEventHandler(
        RtcEngineEventHandler(
          onJoinChannelSuccess: (connection, elapsed) {
            log('Agora joined channel: ${connection.channelId}');
            if (mounted) setState(() => _localUserJoined = true);
          },
          onUserJoined: (connection, remoteUid, elapsed) {
          ...
          onUserOffline: (connection, remoteUid, reason) {
            log('Remote user offline: $remoteUid  reason: $reason');
            // Remote hung up → end locally
            _endCall();
          },
      ...
      AppLogger.info('Joining Agora channel: $_resolvedChannelId');

      // TODO: fetch a real token from your backend instead of hardcoding.
      await _engine!.joinChannel(
        token: 'YOUR_AGORA_TOKEN', // replace with backend-generated token
        channelId: _resolvedChannelId, // ← uses the real channel id
        uid: 0,
        options: const ChannelMediaOptions(
          clientRoleType: ClientRoleType.clientRoleBroadcaster,
          channelProfile: ChannelProfileType.channelProfileCommunication,
  ...
  /// End call: notifies the remote side, leaves Agora, dismisses screen.
  Future<void> _endCall() async {
    _timer?.cancel();
    _controlsHideTimer?.cancel();

    // Tell backend → it pushes FCM type=end_call to the contact
    ...
    // Dismiss any native CallKit overlay still showing
  ...
  /// POST to your backend so it sends FCM end_call to the other user.
  Future<void> _notifyCallEnded() async {
    try {
      // TODO: replace with your actual HTTP call, e.g. using Dio:
      //
      // await dio.post('/api/calls/end', data: {
      //   'recipientId' : widget.contactId,
      //   'channelId'   : _resolvedChannelId,
      // });
      //
      log('Notifying backend: call ended for ${widget.contactId}');
    } catch (e) {
      log('_notifyCallEnded error: $e');
    }
  }
  ...
  @override
  Widget build(BuildContext context) =>
      _isVideoCall ? _buildVideoCallUI() : _buildAudioCallUI();
    ...
    if (_remoteUid != null && _callState == CallState.connected) {
      return AgoraVideoView(
        controller: VideoViewController.remote(
          rtcEngine: _engine!,
          canvas: VideoCanvas(uid: _remoteUid),
          connection: RtcConnection(channelId: _resolvedChannelId),
        ),
      );
    }
    return Container(
    ...
    return Row(
      ...
      children: [
        ...
        _RoundCallBtn(
          icon: _isVideoCall ? Icons.videocam_rounded : Icons.call_rounded,
          color: AppColors.success,
          onTap: _initAgoraAndCall, // ← tapping green starts Agora
        ),
      ],
    );
  }
